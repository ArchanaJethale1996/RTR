...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer,OnGestureListener, OnDoubleTapListener {
	...
	private int[] vbo_Position_CheckerBoard=new int[1];
	private int[] vbo_Texture_CheckerBoard=new int[1];
	...
	private static int numberOfTaps=0;
	...
	private void Initialize()
	{
		...
		GLES32.glGenBuffers(1, vbo_Position_CheckerBoard,0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_Position_CheckerBoard[0]);
		...
		GLES32.glGenBuffers(1, vbo_Texture_CheckerBoard,0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_Texture_CheckerBoard[0]);
	...
	private int LoadTexture()
	{
		...
		System.out.println("RTR: CheckImage Length "+checkImage.length);
	...
	private void MakeCheckImage()
	{
		...
		for (int i = 0; i < CHECK_IMAGE_HEIGHT; i++)
		{
			for (int j = 0; j < CHECK_IMAGE_WIDTH; j++)
			{
					System.out.println("RTR: "+checkImage[((i*64+j)*4)+0]);	
					
					System.out.println("RTR: "+checkImage[((i*64+j)*4)+1]);	
					System.out.println("RTR: "+checkImage[((i*64+j)*4)+2]);	
					//System.out.println("RTR: "+checkImage[((i*64+j)*4)+3]);				
			}
		}
	...
	private void display()
	{
		...
		//Convert the array which can be sent to buffer Data
		ByteBuffer byteBufferTexture=ByteBuffer.allocateDirect(positionCood.length*4);
		//Arrange the buffer in native byte order
		byteBufferTexture.order(ByteOrder.nativeOrder());
		//To Float buffer
		FloatBuffer positionBufferTexture=byteBufferTexture.asFloatBuffer();
		//Put array in Cooked buffer
		positionBufferTexture.put(positionCood);
		//Start from 0th position
		positionBufferTexture.position(0);

		GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER,
			positionCood.length*4,
	...
	{
		if (vbo_Texture_CheckerBoard[0]!=0)
		{
			GLES32.glDeleteBuffers(1, vbo_Texture_CheckerBoard,0);
			vbo_Texture_CheckerBoard[0] = 0;
		}

		if (vbo_Position_CheckerBoard[0]!=0)
		{
			GLES32.glDeleteBuffers(1, vbo_Position_CheckerBoard,0);
			vbo_Position_CheckerBoard[0] = 0;
		}
