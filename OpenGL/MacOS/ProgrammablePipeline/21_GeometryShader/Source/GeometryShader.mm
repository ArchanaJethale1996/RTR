...
-(void)prepareOpenGL
{
    ...
    //Geometry shader
    geomeryShaderObject=glCreateShader(GL_GEOMETRY_SHADER);
    ...
    "}" \
    "}";
    glShaderSource(geomeryShaderObject,1,(const GLchar**)&geometryShaderSourceCode,NULL);
    
    glCompileShader(geomeryShaderObject);
    iInfoLogLength=0;
    iShaderCompileStatus=0;
    szInfoLog=NULL;
    glGetShaderiv(geomeryShaderObject,GL_COMPILE_STATUS,&iShaderCompileStatus);
    if(iShaderCompileStatus==GL_FALSE)
    {
        glGetShaderiv(geomeryShaderObject,GL_INFO_LOG_LENGTH,&iInfoLogLength);
        if(iInfoLogLength>0)
        {
            szInfoLog=(char*)malloc(iInfoLogLength);
            if(szInfoLog!=NULL)
            {
                GLsizei written;
                glGetShaderInfoLog(geomeryShaderObject,iInfoLogLength,&written,szInfoLog);
    ...
    glAttachShader(shaderProgramObject,geomeryShaderObject);
...
-(void) dealloc{
    ...
    glDetachShader(shaderProgramObject,geomeryShaderObject);
