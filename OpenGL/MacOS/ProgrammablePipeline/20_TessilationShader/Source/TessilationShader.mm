...
-(void)prepareOpenGL
{
    ...
    glShaderSource(tessellationControlShaderObject, 1, (const GLchar **)&tessilationControlShaderSourceCode, NULL);
    ...
    if (iShaderCompileStatus == GL_FALSE)
    {
        glGetShaderiv(tessellationControlShaderObject, GL_INFO_LOG_LENGTH, &iInfoLength);
        if (iInfoLength > 0)
        {
            szInfoLog = (GLchar *)malloc(iInfoLength);
            if (szInfoLog != NULL) {
                GLsizei written;
                glGetShaderInfoLog(tessellationControlShaderObject,iInfoLength,&written,szInfoLog);
                fprintf(gpFile,"TessilationControl Shader Compilation Log : %s\n",szInfoLog);
                free(szInfoLog);
                [self release];
                [NSApp terminate:self];
            }
    ...
    //Specify the above source code to the shader object
    glShaderSource(tessellationEvaluationShaderObject, 1, (const GLchar **)&tessillationEvaluationShaderSourceCode, NULL);
    ...
    if (iShaderCompileStatus == GL_FALSE)
    {
        glGetShaderiv(tessellationEvaluationShaderObject, GL_INFO_LOG_LENGTH, &iInfoLength);
        if (iInfoLength > 0)
        {
            szInfoLog = (GLchar *)malloc(iInfoLength);
            if (szInfoLog != NULL) {
                GLsizei written;
                glGetShaderInfoLog(tessellationEvaluationShaderObject,iInfoLength,&written,szInfoLog);
                fprintf(gpFile,"Tessilation Evaluation Shader Compilation Log : %s\n",szInfoLog);
                free(szInfoLog);
                [self release];
                [NSApp terminate:self];
            }
    ...
    gNumberOfLineSegmentsUniform = glGetUniformLocation(shaderProgramObject,"numberOfSegments");
...
-(void)drawView
{
    ...
    translationMatrix=vmath::translate(0.5f, 0.5f, -2.0f);
