...
int initialize(void)
{
	...
	gGeomeryShaderObject = glCreateShader(GL_GEOMETRY_SHADER);

	//Write Shader source code
	GLchar *geometryShaderSourceCode =
		"#version 420 core" \
		"\n" \
		"layout(triangles)in;" \
	...
	//Specify the above source code to the shader object
	glShaderSource(gGeomeryShaderObject, 1, (const GLchar **)&geometryShaderSourceCode, NULL);

	//compile the vertex shader source code
	glCompileShader(gGeomeryShaderObject);
	...
	glGetShaderiv(gGeomeryShaderObject, GL_COMPILE_STATUS, &iShaderCompileStatus);
	if (iShaderCompileStatus == GL_FALSE)
	{
		glGetShaderiv(gGeomeryShaderObject, GL_INFO_LOG_LENGTH, &iInfoLength);
		if (iInfoLength > 0)
		{
			szInfoLog = (GLchar *)malloc(iInfoLength);
			if (szInfoLog != NULL) {
				GLsizei written;
				glGetShaderInfoLog(gGeomeryShaderObject, iInfoLength, &written, szInfoLog);
				fprintf(gpFile, szInfoLog);
				free(szInfoLog);
				unInitialize();
				DestroyWindow(ghwnd);
	...
	glAttachShader(gShaderProgramObject, gGeomeryShaderObject);
