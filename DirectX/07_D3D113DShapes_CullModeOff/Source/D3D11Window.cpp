...
HRESULT initialize(void)
{
	...
	D3D11_BUFFER_DESC bufferDesc_color_Pyramid;
	ZeroMemory(&bufferDesc_color_Pyramid, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_color_Pyramid.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_color_Pyramid.ByteWidth = sizeof(float)*ARRAYSIZE(verticesColor);
	bufferDesc_color_Pyramid.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_color_Pyramid.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = gpD3D11Device->CreateBuffer(&bufferDesc_color_Pyramid, NULL, &gpID3D11Buffer_VertexBuffer_Color_Pyramid);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, " gpD3D11Device->CreateBuffer() Failed For VertexShader.\n");
	...
	//D3D11_MAPPED_SUBRESOURCE mappedSubresource;
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Color_Pyramid, NULL, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, verticesColor, sizeof(verticesColor));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Color_Pyramid, 0);
	...
	bufferDesc_position_Cube.ByteWidth = sizeof(float)*ARRAYSIZE(verticesCube);
	...
	D3D11_BUFFER_DESC bufferDesc_color_Cube;
	ZeroMemory(&bufferDesc_color_Cube, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_color_Cube.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_color_Cube.ByteWidth = sizeof(float)*ARRAYSIZE(verticesColorCube);
	bufferDesc_color_Cube.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_color_Cube.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = gpD3D11Device->CreateBuffer(&bufferDesc_color_Cube, NULL, &gpID3D11Buffer_VertexBuffer_Color_Cube);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, " gpD3D11Device->CreateBuffer() Failed For VertexShader.\n");
	...
	//D3D11_MAPPED_SUBRESOURCE mappedSubresource;
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Color_Cube, NULL, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, verticesColorCube, sizeof(verticesColorCube));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Color_Cube, 0);
...
void display(void)
{
	...
	rotationMatrix = XMMatrixRotationX(gAngleCube)*XMMatrixRotationY(gAngleCube)*XMMatrixRotationZ(gAngleCube);
	worldMatrix = rotationMatrix*translateMatrix;
