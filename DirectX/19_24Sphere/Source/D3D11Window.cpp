...
void display(void)
{
	...
	XMMATRIX translateMatrix = XMMatrixTranslation(0.0f, 0.0f, 2.0f);
	...
	for (int i = 0; i < 4; i++)
	{
		...
		for (int j = 0; j < 6; j++)
		{
			d3dViewPort.TopLeftX = (float)(gWidth / 6) * j;
			d3dViewPort.TopLeftY = (float)(gHeight / 4) * i;
			...
			if (gLighting == 1)
			{
				...
				constantBuffer.Kd = XMVectorSet(materialDiffused[i][j][0], materialDiffused[i][j][1], materialDiffused[i][j][2], materialDiffused[i][j][3]);
				constantBuffer.Ka = XMVectorSet(materialAmbient[i][j][0], materialAmbient[i][j][1], materialAmbient[i][j][2], materialAmbient[i][j][3]);
				constantBuffer.Ks = XMVectorSet(materialSpecular[i][j][0], materialSpecular[i][j][1], materialSpecular[i][j][2], materialSpecular[i][j][3]);
				constantBuffer.MaterialShininess = materialShininess[i][j];
				if (KeyPressed == 1)
				{
					constantBuffer.LightPosition = XMVectorSet(0.0f, sinf(gAngleSphere) * 100.0f - 3.0f, cosf(gAngleSphere) * 100.0f - 3.0f, 1.0f);
				}

				if (KeyPressed == 2)
				{
					constantBuffer.LightPosition = XMVectorSet(sinf(gAngleSphere) * 100.0f - 3.0f, 0.0f, cosf(gAngleSphere) * 100.0f - 3.0f, 1.0f);
				}

				if (KeyPressed == 3)
				{
					constantBuffer.LightPosition = XMVectorSet(sinf(gAngleSphere) * 100.0f - 3.0f, cosf(gAngleSphere) * 100.0f - 3.0f, 0.0f, 1.0f);
				}
