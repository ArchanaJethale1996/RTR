...
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Position_Smiley = NULL;
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Texture_Smiley = NULL;

ID3D11ShaderResourceView *gpID3D11ShaderResourceView_Texture_White = NULL;
ID3D11SamplerState *gpID3D11SamplerState_Texture_White = NULL;
...
HRESULT initialize(void)
{
	...
	D3D11_BUFFER_DESC bufferDesc_position_Smiley;
	ZeroMemory(&bufferDesc_position_Smiley, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_position_Smiley.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_position_Smiley.ByteWidth = sizeof(float)*ARRAYSIZE(vertices);
	bufferDesc_position_Smiley.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_position_Smiley.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = gpD3D11Device->CreateBuffer(&bufferDesc_position_Smiley, NULL, &gpID3D11Buffer_VertexBuffer_Position_Smiley);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Position_Smiley, NULL, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, vertices, sizeof(vertices));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Position_Smiley, 0);


	D3D11_BUFFER_DESC bufferDesc_texture_Smiley;
	ZeroMemory(&bufferDesc_texture_Smiley, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_texture_Smiley.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_texture_Smiley.ByteWidth = sizeof(float)*12;
	bufferDesc_texture_Smiley.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_texture_Smiley.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = gpD3D11Device->CreateBuffer(&bufferDesc_texture_Smiley, NULL, &gpID3D11Buffer_VertexBuffer_Texture_Smiley);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, " gpD3D11Device->CreateBuffer() Failed For VertexShader.\n");
	...
	hr = gpD3D11Device->CreateSamplerState(&samplerDesc, &gpID3D11SamplerState_Texture_Smiley);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
	...
	samplerDesc.AddressV = D3D11_TEXTURE_ADDRESS_WRAP;
	samplerDesc.AddressW = D3D11_TEXTURE_ADDRESS_WRAP;
	hr = gpD3D11Device->CreateSamplerState(&samplerDesc, &gpID3D11SamplerState_Texture_White);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "gpD3D11Device->CreateSamplerState() Failed.\n");
...
HRESULT LoadD3DTextureWhite()
{
	...
	HRESULT hr = DirectX::CreateWICTextureFromMemory(gpD3D11Device, gpID3D11DeviceContext, (const uint8_t*)bitmapImage, (size_t)totalBitmapImageSize, NULL, &gpID3D11ShaderResourceView_Texture_White, NULL);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
	...
	hr = gpD3D11Device->CreateSamplerState(&samplerDescriptor, &gpID3D11SamplerState_Texture_White);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
...
void display(void)
{
	if (keyPressed == 0)
	{
		...
		D3D11_MAPPED_SUBRESOURCE mappedSubresource;
		ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
		gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Texture_Smiley, NULL, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
		memcpy(mappedSubresource.pData, verticesTexccord, sizeof(verticesTexccord));
		gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Texture_Smiley, 0);
	...
	gpID3D11DeviceContext->IASetVertexBuffers(0
		, 1
		, &gpID3D11Buffer_VertexBuffer_Position_Smiley
		, &stride,
		&offset);
	...
	gpID3D11DeviceContext->IASetVertexBuffers(1,
		1,
		&gpID3D11Buffer_VertexBuffer_Texture_Smiley,
		&stride,
		&offset);
	...
	if (keyPressed == 3)
	{
		...
		gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_Texture_White);
		gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpID3D11SamplerState_Texture_White);
	}
	else
	{
		gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_Texture_Smiley);
		gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpID3D11SamplerState_Texture_Smiley);
	}
...
void unInitialize(void)
{
	...
	if (gpID3D11Buffer_VertexBuffer_Position_Smiley)
	{
		gpID3D11Buffer_VertexBuffer_Position_Smiley->Release();
		gpID3D11Buffer_VertexBuffer_Position_Smiley = NULL;
	}

	if (gpID3D11Buffer_VertexBuffer_Texture_Smiley)
	{
		gpID3D11Buffer_VertexBuffer_Texture_Smiley->Release();
		gpID3D11Buffer_VertexBuffer_Texture_Smiley = NULL;
	}
