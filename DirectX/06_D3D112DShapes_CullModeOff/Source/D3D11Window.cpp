...
HRESULT initialize(void)
{
	...
	D3D11_BUFFER_DESC bufferDesc_position_Rectangle;
	ZeroMemory(&bufferDesc_position_Rectangle, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_position_Rectangle.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_position_Rectangle.ByteWidth = sizeof(float)*ARRAYSIZE(verticesRectangle);
	bufferDesc_position_Rectangle.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_position_Rectangle.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = gpD3D11Device->CreateBuffer(&bufferDesc_position_Rectangle, NULL, &gpID3D11Buffer_VertexBuffer_Position_Rectangle);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, " gpD3D11Device->CreateBuffer() Failed For VertexShader.\n");
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Position_Rectangle, NULL, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, verticesRectangle, sizeof(verticesRectangle));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Position_Rectangle, 0);
	...
	bufferDesc_color_Rectangle.ByteWidth = sizeof(float)*ARRAYSIZE(verticesColorRectangle);
	...
	hr = gpD3D11Device->CreateBuffer(&bufferDesc_color_Rectangle, NULL, &gpID3D11Buffer_VertexBuffer_Color_Rectangle);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, " gpD3D11Device->CreateBuffer() Failed For VertexShader.\n");
	...
	//D3D11_MAPPED_SUBRESOURCE mappedSubresource;
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Color_Rectangle, NULL, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, verticesColorRectangle, sizeof(verticesColorRectangle));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Color_Rectangle, 0);
...
void display(void)
{
	...
	rotationMatrix = XMMatrixRotationX(gAngleRectangle);
	worldMatrix = rotationMatrix*translateMatrix;
...
void update(void)
{
	...
	gAngleRectangle = gAngleRectangle - 0.001f;
	if (gAngleRectangle <= 0.0f)
	{
		gAngleRectangle = 360.0f;
	}
}
