...
HRESULT initialize(void)
{
	...
	bufferDesc_color_Cube.ByteWidth = sizeof(float)*ARRAYSIZE(verticesNormalCube);
	bufferDesc_color_Cube.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_color_Cube.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = gpD3D11Device->CreateBuffer(&bufferDesc_color_Cube, NULL, &gpID3D11Buffer_VertexBuffer_Normal_Cube);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, " gpD3D11Device->CreateBuffer() Failed For VertexShader.\n");
	...
	//D3D11_MAPPED_SUBRESOURCE mappedSubresource;
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Normal_Cube, NULL, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, verticesNormalCube, sizeof(verticesNormalCube));
