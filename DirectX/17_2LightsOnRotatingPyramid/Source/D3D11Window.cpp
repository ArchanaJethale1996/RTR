...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCodePerVertex =
		...
		"       output.phongLight = float4(ambientOne+diffusedOne+specularOne+ambientTwo+diffusedTwo+specularTwo,1.0);" \
		"}" \
		"else" \
		"{" \
	...
	const char* pixelShaderSourceCode_PerFragment =
		...
		"float3 phong_ads_light = ambientTwo+diffusedTwo+specularTwo+ambientOne+diffusedOne+specularOne;" \
	...
	float verticesNormal[] = {
		...
		0.0f, 0.447214f, -0.8944270f,
		0.0f, 0.447214f, -0.8944270f,
		0.0f, 0.447214f, -0.8944270f,

		-0.8944270f, 0.447214f, 0.0f,
		-0.8944270f, 0.447214f, 0.0f,
		-0.8944270f, 0.447214f, 0.0f,
	};
	...
	bufferDesc_position_Pyramid.ByteWidth = sizeof(float)*_ARRAYSIZE(vertices);
	...
	D3D11_BUFFER_DESC bufferDesc_normal_Pyramid;
	ZeroMemory(&bufferDesc_normal_Pyramid, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_normal_Pyramid.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_normal_Pyramid.ByteWidth = sizeof(float)*ARRAYSIZE(verticesNormal);
	bufferDesc_normal_Pyramid.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_normal_Pyramid.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = gpD3D11Device->CreateBuffer(&bufferDesc_normal_Pyramid, NULL, &gpID3D11Buffer_VertexBuffer_Normal_Pyramid);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, " gpD3D11Device->CreateBuffer() Failed For VertexShader.\n");
	...
	//D3D11_MAPPED_SUBRESOURCE mappedSubresource;
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Normal_Pyramid, NULL, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, verticesNormal, sizeof(verticesNormal));
...
void display(void)
{
	...
	XMMATRIX r1 = XMMatrixRotationY(gAnglePyramid);
	XMMATRIX rotationMatrix = r1;
	...
	if (gLighting == 1)
	{
		...
		constantBuffer.LdOne = XMVectorSet(1.0f, 0.0f, 0.0f, 1.0f);
		constantBuffer.LaOne = XMVectorSet(1.0f, 0.0f, 0.0f, 1.0f);
		constantBuffer.LsOne = XMVectorSet(1.0f, 0.0f, 0.0f, 1.0f);
		constantBuffer.LdTwo = XMVectorSet(0.0f, 0.0f, 1.0f, 1.0f);
		constantBuffer.LaTwo = XMVectorSet(0.0f, 0.0f, 1.0f, 1.0f);
		constantBuffer.LsTwo = XMVectorSet(0.0f, 1.0f, 1.0f, 1.0f);
		...
		constantBuffer.LightPositionOne = XMVectorSet(-2.0f, 0.0f, 0.0f, 1.0f);
		constantBuffer.LightPositionTwo = XMVectorSet(2.0f, 0.0f, 0.0f, 1.0f);
...
void update(void)
{
	...
	if (gAnglePyramid <= 0.0f)
	{
		gAnglePyramid = 360.0f;
	}
}
